...
def s2_authors_match(authors: list[str], data: dict) -> bool:
    ...
    def filtered_author_names(names):
        return (" ".join(w for w in name.split() if len(w) > AUTHOR_NAME_MIN_LENGTH) for name in names)

    s2_authors_noinit = filtered_author_names(a["name"] for a in data["authors"])
    authors_noinit = filtered_author_names(authors)
