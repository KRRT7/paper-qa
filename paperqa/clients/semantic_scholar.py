...
def s2_authors_match(authors: list[str], data: dict) -> bool:
    ...
    def filter_author_name(name: str):
        return " ".join(w for w in name.split() if len(w) > AUTHOR_NAME_MIN_LENGTH)

    s2_authors_noinit = (filter_author_name(a["name"]) for a in data["authors"])
    authors_noinit = (filter_author_name(a) for a in authors)
